...
function EpisodeCard({
  ...
  if (usersData.length === 0) {
    ...
    isFavoriteCard = usersData[userId].episodesData.includes(episode.id)
  }
  ...
  function setHandler(e: any) {
    if (isFavorite) {
      const index = usersData[userId].episodesData.indexOf(episode.id)
      if (index !== -1) {
        dispatch(
          removeEpisodeData({
            episodesData: index,
      ...
      <div className='flex flex-col px-[30px] py-[11px] w-full'>
        ...
        <div className='flex flex-wrap justify-between'>
          ...
          {/* <ul className='font-medium text-xs flex flex-col gap-y-[5px] max-w-[283px]'>
            ...
            <li className='text-4xl'>{episode.characters}</li>
          </ul> */}
        </div>
      </div>
